    ...
    try:
        ...
        if input_json.get('isSuccess') == 'true' and input_json.get('ReferenceNumber'):
            ...
            while time.time() - start_time <= 300:  # Limit status check to 5 minutes (300 seconds)
                ...
                if status_response.ok and status_response.text:  # Check if response is valid and not empty
                    try:
    ...
    if os.path.isdir(folder_path):
        ...
        if os.path.exists(input_folder):
            ...
            for input_file in input_files:
                ...
                success = send_input_file_and_get_status(input_file, headers, input_api_endpoint, status_api_endpoint, download_api_endpoint)
                if success:
