...
def send_input_file_and_get_status(input_file_path, headers, input_api_url, status_api_url, download_api_url):
    try:
        ...
        input_response.raise_for_status()  # Raise exception for HTTP errors

        input_json = input_response.json()

        if input_json.get('isSuccess') == 'true' and input_json.get('Message') == 'Package received':
            start_time = time.time()
            while time.time() - start_time <= 300:  # Limit status check to 5 minutes (300 seconds)
                status_response = requests.post(status_api_url, =headers['status'], json={'UserId': 206})
                status_response.raise_for_status()  # Raise exception for HTTP errors

                if status_response.ok and status_response.text:  # Cheadersheck if response is valid and not empty
                    try:
                        ...
                        for item in status_json.get('dtoStatus', []):
                            if item.get('ProjectName') == project_name and item.get('Status') == 'Output & Report':
                                ...
                                download_response.raise_for_status()  # Raise exception for HTTP errors
                    ...
                    print("Empty or invalid status response")
                    # Handle the case when the response is empty or invalid
...
# Function to send input file and get status (with delay)
def send_input_file_and_get_status_with_delay(input_file_path, headers, input_api_url, status_api_url, download_api_url):
    success = send_input_file_and_get_status(input_file_path, headers, input_api_url, status_api_url, download_api_url)
    if success:
    ...
    if os.path.isdir(folder_path):
        ...
        if os.path.exists(input_folder):
            ...
            for input_file in input_files:
                ...
                send_input_file_and_get_status_with_delay(input_file, headers, input_api_endpoint, status_api_endpoint, download_api_endpoint)
                time.sleep(15)  # Add a 15-second delay after processing each file
