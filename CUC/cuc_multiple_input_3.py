...
def upload_zip_to_postman(job_id, project_name, api_key, zip_file_path):
    ...
    headers = {
        ...
        'type': 'VENDOR',
        'UserId': '206',
        'ProjectName': project_name,  # Use project_name as ProjectName
        'RefStyle': 'None'
    }
    ...
    if response.status_code == 201:
        print(f"File uploaded successfully for Job ID {job_id} and Project Name {project_name}: {response.text}")
    else:
        print(f"Failed to upload file for Job ID {job_id} and Project Name {project_name}. Error: {response.text}")
...
for root, dirs, files in os.walk(directory_path):
    for file in files:
        if file.endswith('.zip'):
            job_id = os.path.basename(file)  # Using directory name as job_id
            project_name = f'{job_id}'  # Using directory name as project_name
            file_path = os.path.join(root, file)
            upload_zip_to_postman(job_id, project_name, api_key, file_path)
